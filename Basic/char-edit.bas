...
470 IF KP$=CHR$(83) THEN GOSUB 36000: REM S KEY TO SAVE
...
2210 IF UI=1 THEN GOSUB 2300
2220 IF UI=2 THEN GOSUB 2500
2230 IF UI=3 THEN GOSUB 2700
2240 IF UI=4 THEN GOSUB 2900
2250 RETURN

2300 REM SPACE BAR PRESSED IN CHAR BOX
2310 REM SPACE WILL TOGGLE "PIXEL" ON OR OFF
2320 X=CC%(0): Y=CC%(1)
2330 IF CB%(X,Y)=0 THEN CB%(X,Y)=1: GOTO 2350
2340 IF CB%(X,Y)=1 THEN CB%(X,Y)=0: GOTO 2350
2350 REM REDRAW CHAR
2360 GOSUB 32000
2370 REM UPDATE VERA
2380 GOSUB 23200
2390 RETURN
...
32040 IF CB%(X,Y)=1 THEN VPOKE 0,$4102+(X*2)+(Y*256),160: T=PS
32050 IF CB%(X,Y)=0 THEN VPOKE 0,$4102+(X*2)+(Y*256),32 : T=0
...
36000 REM SAVE FILE
36010 REM MOVE CURSOR TO FILE-NAME BOX AND ASK FOR INPUT
36020 REM NOT ADJUSTING THE ACCUMULATOR
36030 POKE 781,8: REM SET THE X REGISTER, WITH SCREEN Y
36040 POKE 782,11:  REM SET THE Y REGISTER, WITH SCREEN X
36050 POKE 783,0:  REM SET THE STATUS REGISTER
36060 SYS $FFF0 :REM USE KERNAL PLOT FUNCITON
36070 INPUT "SAVE=";FO$
36080 FO$ = "@0:" + FO$ + ".CHAR,S,W"
36090 OPEN 8,8,8,FO$
36100 FOR I=0 TO 511
36110 LET T=VPEEK(1,PR+I)
36120 PRINT#8, T
36130 NEXT I
36140 CLOSE 8
36150 LET DF$=FO$: RE
